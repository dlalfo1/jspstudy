...
public class BoardDAO {
	...
	public int insertBoard(BoardDTO board) {
		...
		try {
			...
			// 변수처리할 때 ? 물음표 쓰기로 함. (title, content는 board에 담겨있으므로 변수처리할거임)
			...
			close();	// 예외 발생 여부와 상관 없이 항상 자원의 반납을 해야 한다.
		}
		
		// 7. 삽입 결과 반환
		return insertResult; // 0 or 1 반환
	...
	public int updateBoard(BoardDTO board) {
		...
		try {
			...
			// 6.PreparedStatement 객체를 이용해 쿼리문 실행(DPDATE문 실행은 executeUpdate 메소드로 한다.)
	...
	public int deleteBoard(int board_no) {
		...
		try {
			...
			// 변수처리할 때 ? 물음표 쓰기로 함. (board_no에 게시글번호가 담겨있으므로 변수처리할거임)
					
			// 4. 쿼리문을 실행할 PreparedStatement 객체 생성
			ps = con.prepareStatement(sql);		
			
			// 5. 쿼리문에 변수처리 하기
			ps.setInt(1, board_no);    // 1번째 물음표(?)에 매개변수로 받은 board_no 전달학.ㅏ
