...
public class BoardModifyService implements IBoardService {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) {
		...
		int boardNo = Integer.parseInt(strBoard_no.isBlank() ? "0" : strBoard_no);
		
		// 2. 요청 파라미터 이용해서 BoardDTO board 객체 생성
		...
		// 4. 어디로 and 어떻게 이동
		return new ActionForward(request.getContextPath() + "/getBoardByNo.do?board_no=" + boardNo, true);
	}
