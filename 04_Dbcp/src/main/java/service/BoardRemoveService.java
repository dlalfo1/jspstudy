...
public class BoardRemoveService implements IBoardService {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) {
		...
		int board_no = Integer.parseInt(strBoard_no.isEmpty() ? "0" : strBoard_no); // board_no가 오지 않았다면 0을 반환하겠다.
		                                                  // 게시글 번호엔 0이 없으므로 삭제작업이 진행되지 않는다.
		// 2. boardDAO에 deleteBoard 메소드 호출
