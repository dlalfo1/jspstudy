...
public class BoardDetailService implements IBoardService {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) {
		...
		BoardDTO board = BoardDAO.getInstance().selectByNo(board_no);
		
		// 3. 존재하지 않는 게시글인 겨우 응답 처리
		...
		// 4. DB에서 가져온 게시글(BoardDTO board)를 request에 저장(상세보기화면(board/detail.jsp)으로 forward(전달)하기 위해서)
