...
<body>
	...
	<% } else { %>
		<div>장바구니 목록</div>
		<% for(int i = 0; i < cart.size(); i++){ %> <!-- ArrayList 길이는 size()  -->
			<div><%=cart.get(i).get("item")%> <%=cart.get(i).get("itemCount")%>개</div>
			<!--
				 get(i) : ArrayList에서 Map꺼내오기
				 get("item") : Map에서 item에 해당하는 값 꺼내오기	
			 -->
			<% } %>
	<% } %>		
	...
	<div>
		<input type="button" value="쇼핑계속하기" onclick="goShopping()">
